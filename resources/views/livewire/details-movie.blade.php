<div>
    <nav class="fixed top-0 w-full z-20 px-4 py-4 flex justify-between items-center bg-transparent">
    ...
    <section class="header-mobile xl:hidden">
        <div class="h-screen bg-cover bg-center xl:bg-top bg-no-repeat w-full"
            ...
            <div class="absolute bottom-10 px-8 flex justify-center w-full">
                <div class="xl:w-[50%]">
                    <h1 class="text-4xl text-white font-bold">{{ $data['title'] }}</h1>
                    <div class="flex items-center gap-1 py-2">
                        <img src="{{ asset('img/bintang.png') }}" alt="" class="w-[10%] xl:w-[2%]">
                        <p class="text-white font-semibold">{{ $data['vote_average'] * 10 . '%' }} | {{ \Carbon\Carbon::parse($data['release_date'])->format('M d, Y') }}</p>
                    </div>
                    <p class="text-base text-slate-300 font-semibold">
                        @foreach ($dataGenre as $genre => $name)
                            {{ $name['name'] . ',' }}
                        @endforeach
                    </p>

                    <p class="text-slate-200 my-2 font-semibold xl:w-1/2">
                        {{ $data['overview'] }}</p>

                    <div class="pt-3 flex gap-5">
                        <button
                            class="bg-blue-500 text-white px-3 py-2 font-semibold rounded-sm hover:bg-blue-600 flex items-center justify-center gap-2">
                            <img src="{{ asset('img/play-3.png') }}" alt="" class="w-[20%]">
                            Play Now
                        </button>
                        <button
                            class="bg-transparent shadow-2xl border border-slate-500 text-white px-5 py-2 font-semibold rounded-sm hover:bg-blue-600">Trailer</button>
                    </div>
                </div>

                <!-- <div class="w-[50%] mx-auto hidden xl:block">
                    <h1 class="text-white text-xl font-semibold">Posters</h1>
                    <div class="flex gap-5">
                        <img src="../img/posters-dune-1.jpg" alt="" class="w-[20%] py-3 rounded-2xl">
                        <img src="../img/posters-dune-2.jpg" alt="" class="w-[20%] py-3 rounded-2xl">
                    </div>
                </div> -->
            </div>
        </div>
            ...
            <div class="flex justify-center gap-10 items-center">
                ...
                <div class="w-[40%]">
                    ...
                    <div class="flex items-center gap-2 my-3">
                        <img src="{{ asset('img/bintang.png') }}" alt="" class="w-[3%]">
                        <p class="text-white font-semibold text-sm ">{{ $data['vote_average'] * 10 . '%' }} | {{ \Carbon\Carbon::parse($data['release_date'])->format('M d, Y') }}</p>
                    </div>
                    <p class="text-base text-slate-300 font-semibold">
                        @foreach ($dataGenre as $genre => $name)
                            {{ $name['name'] . ',' }}
                    ...
                    <p class="text-slate-200 my-2 font-semibold">
                        {{ $data['overview'] }}</p>
